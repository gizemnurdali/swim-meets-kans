...
algorithm for pre-trained ResNet50. 

Target task: ImageNet

Source task: CIFAR-10

Pre-trained neural network architecture: ResNet50

Weights and biases of the classification head: 
Iterative training with Adam optimizer

Two output files: 
(1) 'ResNet50_training.csv' file with accuracy on training data, test data and sampling time for different widths
...
# Preprocessing data
x_train_1 = tf.keras.applications.resnet50.preprocess_input(x_train)
x_test_1 = tf.keras.applications.resnet50.preprocess_input(x_test)
...
base_model_retraining = ResNet50(include_top=False,weights='imagenet',input_shape=(224, 224,3),classes=y_train.shape[1])
...
for a in range(len(activation_functions)):
    for i in range(len(widths)):
        ...
        # Summary
        print('Summary: Pre-trained neural network - ResNet50, Dataset: CIFAR-10, Head Weights - Trained')
...
data.to_csv('ResNet50_training.csv', index=False)
