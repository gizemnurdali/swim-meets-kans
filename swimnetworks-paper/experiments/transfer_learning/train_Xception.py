...
algorithm for pre-trained Xception. 
...
Weights and biases of the classification head: Iterative training with Adam optomizer

Two output files: 
(1) 'Xception_training.csv' file with accuracy on training data, test data and sampling time for different widths
...
# Preprocessing data - Inputs pixel values are scaled between -1 and 1, sample-wise
x_train_1 = tf.keras.applications.xception.preprocess_input(x_train)
x_test_1 = tf.keras.applications.xception.preprocess_input(x_test)
...
for a in range(len(activation_functions)):
    for i in range(len(widths)):
        ...
        # Summary
        print('Summary: Pre-trained neural network - Xception, Dataset: CIFAR-10, Head Weights - Trained')
...
data.to_csv('Xception_training.csv', index=False)
