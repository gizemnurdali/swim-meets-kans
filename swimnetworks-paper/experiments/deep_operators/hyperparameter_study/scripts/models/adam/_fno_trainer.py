...
class Trainer:
    def __init__(self, model, n_epochs, patience=None, wandb_log=True, device=None,
    ...
    def train(self, train_loader, test_loaders, output_encoder,
        ...
        for epoch in range(self.n_epochs):
            ...
            if epoch % self.log_test_interval == 0: 
                ...
                # Use erros for the last test loader
                metric = errors[test_error_name]
                if metric < best_error:
                    best_error = metric
                    best_epoch = epoch
            ...
            if (epoch - best_epoch) > self.patience:
                print(f"Early stopping after {epoch} epochs: {best_epoch=}")
                if best_state_dict is not None:
                    model.load_state_dict(best_state_dict)
                else:
                    print("WARNING: couldn't find the best state dict.")
                break

    def evaluate(self, model, loss_dict, data_loader, output_encoder=None,
                 log_prefix='', return_predictions=False):
        ...
        if return_predictions:
            ...
            return predictions.cpu().detach().numpy(), errors
        else:
            del x, y, out
            return errors
