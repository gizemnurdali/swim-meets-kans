...
class Base(BaseEstimator, ABC):
    ...
    biases: np.ndarray = field(default_factory=lambda: np.empty((0, 0)))
    n_parameters: int = 0

    input_shape: Optional[Tuple[int, ...]] = None
    output_shape: Optional[Tuple[int, ...]] = None
    _activation_func: Optional[Callable[[np.ndarray], np.ndarray]] = None
    ...
    def __post_init__(self):
        ...
        if isinstance(self.activation, str):
            if self.activation == "none" or self.activation is None:
                self._activation_func = Base.identity_activation
            elif self.activation == "relu":
                self._activation_func = Base.relu_activation
            elif self.activation == "tanh":
                self._activation_func = Base.tanh_activation
            else:
                raise ValueError(f"Unknown activation {self.activation}.")
        else:
            self._activation_func = self.activation
    ...
    def transform(self, x, y=None):
        ...
        if self._activation_func is None:
            raise ValueError("The activation function was not initialized.")

        x = self.prepare_x(x)
        result = self._activation_func(x @ self.weights + self.biases)
        return result
    ...
    def prepare_y_inverse(self, y):
        ...
        if self._classes is None:
            raise ValueError("Class labels are not set. Ensure the estimator is fitted before calling prepare_y_inverse.")
