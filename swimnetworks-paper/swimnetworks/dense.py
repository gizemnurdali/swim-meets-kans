...
class Dense(Base):
    ...
    random_seed: int = 1
    dist_min: float = 1e-10
    repetition_scaler: int = 1

    idx_from: Optional[np.ndarray] = None
    idx_to: Optional[np.ndarray] = None
    _sampler_func: Optional[Callable] = None

    def __post_init__(self):
        super().__post_init__()

        if isinstance(self.parameter_sampler, str):
            if self.parameter_sampler == "relu":
                self._sampler_func = self.sample_parameters_relu
            elif self.parameter_sampler == "tanh":
                self._sampler_func = self.sample_parameters_tanh
            elif self.parameter_sampler == "random":
                self._sampler_func = self.sample_parameters_randomly
            else:
                raise ValueError(f"Unknown parameter sampler {self.parameter_sampler}.")
        else:
            self._sampler_func = self.parameter_sampler
    ...
    def fit(self, x, y=None):
        ...
        if self._sampler_func is None:
            raise ValueError("Sampler function not initialized.")
        weights, biases, idx_from, idx_to = self._sampler_func(x, y, rng)
    ...
    def sample_parameters(self, x, y, rng):
        ...
        candidates_idx_from = rng.integers(low=0, high=x.shape[0], size=x.shape[0]*n_repetitions) # choose M points
        delta = rng.integers(low=1, high=x.shape[0]-1, size=candidates_idx_from.shape[0]) # choose M points again
        candidates_idx_to = (candidates_idx_from + delta) % x.shape[0] # use delta and calculate new indices
        
        directions = x[candidates_idx_to, ...] - x[candidates_idx_from, ...] # elementwise row-by-row subtraction
        dists = np.linalg.norm(directions, axis=1, keepdims=True) # l2 norm calculation
        dists = np.clip(dists, a_min=self.dist_min, a_max=None) # enforce a range on every element
        directions = directions / dists # normalize direction (M, D) matrices by dists

        dy = y[candidates_idx_to, :] - y[candidates_idx_from, :] # difference in target values
        if self.is_classifier: # sign is irrelevant, only whether the value is zero or nonzero matters
            dy[np.abs(dy) > 0] = 1
        ...
        selected_idx = rng.choice(dists.shape[0], # sample N_l winners from M candidates using calculated probabilities
    ...
    def weight_probabilities(self, dy, dists):
        ...
        # L∞ norm:  max(|v1|, |v2|, ..., |vD|)
        # l2 aggregates all dimensions, while L∞ takes the largest possible
        ...
        if self.sample_uniformly or np.sum(gradients) < self.dist_min:
            ...
            probabilities = np.ones_like(gradients) / len(gradients) # everyone get 1/M proba
        else:
            probabilities = gradients / np.sum(gradients) # normalize so all probabilities sum to 1
